...
//Foil is a struct contain foil data
type Foil struct {
	name string
	Mach []float64
	Data []foildata
...
//Readfoil reads foil
func Readfoil(name string) Foil {
	var (
		dir   string
		foil  Foil
		M     []float64
	...
	for i, f := range files {
		filename[i] = dir + f.Name()
		//fmt.Println(file_name[i])
		file, err1 := os.Open(filename[i])
		defer file.Close()
		if err1 != nil {
			fmt.Printf("Error")
		}
	...
	for i := 0; i < len(fls); i++ {
		file, err1 := os.Open(fls[i].FileName)
		defer file.Close()
		if err1 != nil {
			fmt.Printf("Error")
		}
...
//FindCy by angle of attack and Mach number
...
//FindCx by angle of attack and Mach number
